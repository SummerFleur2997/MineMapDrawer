...
public static class Program
{
    ...
    private static int CompletedActions => _completedActions;
    public static void IncrementCompletedActions() => Interlocked.Increment(ref _completedActions);
    ...
    public static void Main()
    {
        ...
        {
            Console.WriteLine($"读取资源文件失败: {ex.Message}");
    ...
    private static void DisplayProgress()
    {
        ...
        while (CompletedActions < TotalActions)
        {
            lock (Lock)
            {
                var percent = CompletedActions * 100.0 / TotalActions;

                if (percent > lastPercent)
                {
                    Console.Write("\r");
                    var filledBlocks = (int)(percent / 2); 
                    Console.Write($"进度: [{new string('█', filledBlocks)}{new string(' ', 50 - filledBlocks)}] {percent:F1}%");
                    lastPercent = percent;
                }
            }
        ...
        Console.Write("\r");
        Console.Write($"进度: [{new string('█', 50)}] 100.0%\n");
    }
